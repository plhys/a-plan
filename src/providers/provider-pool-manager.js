...
export class ProviderPoolManager {
    ...
    async _checkProviderHealth(providerType, providerConfig) {
        ...
        // Cloudflare Gateway 是路由层，如果没有配置 checkModelName 或 supportedModels，提示用户配置
        if ((providerType === 'cloudflare-gateway-free' || providerType === 'cloudflare-gateway-proxy') && !modelName) {
            this._log('warn', `Cloudflare Gateway (${providerType}) requires a check model to be configured. Please set checkModelName in provider config.`);
            return { 
                success: false, 
                modelName: null, 
                errorMessage: 'Cloudflare Gateway 需要配置检查模型。请在管理后台设置 "检查模型名称" (checkModelName)，例如：qwen-max、glm-4 等实际可用的模型。' 
            };
        }
